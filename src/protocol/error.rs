...
impl From<crate::protocol::message::MessageError> for ProtocolError {
    fn from(err: crate::protocol::message::MessageError) -> Self {
        match err {
            crate::protocol::message::MessageError::InvalidFormat => {
                ProtocolError::InvalidMessage("Invalid message format".to_string())
            }
            crate::protocol::message::MessageError::InvalidMagic => {
                ProtocolError::InvalidMagic {
                    expected: 0, // Will be filled in by caller
                    actual: 0,
                }
            }
            crate::protocol::message::MessageError::DataLengthMismatch => {
                ProtocolError::DataLengthMismatch {
