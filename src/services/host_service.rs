...
impl HostService {
    ...
    pub async fn execute_command(&self, command: HostCommand) -> Result<HostServiceResponse> {
        match command {
            ...
            HostCommand::Transport(device_id) => {
                self.select_device(&device_id).await
            }
            HostCommand::TransportAny => {
                self.select_any_device().await
            }
        }
    }
        ...
        match transport_info {
            Some((_, is_connected)) => {
                if is_connected {
                    ...
                    Ok(HostServiceResponse::TransportSelected(device_id.to_string()))
                } else {
    ...
    async fn select_any_device(&self) -> Result<HostServiceResponse> {
        ...
        for device_id in transport_ids {
            if let Some((_, is_connected)) = self.transport_manager.get_transport_info(&device_id).await {
                if is_connected {
...
mod tests {
    ...
    async fn test_select_nonexistent_device() {
        ...
        let result = service.execute_command(
            HostCommand::Transport("nonexistent".to_string())
        ).await;
